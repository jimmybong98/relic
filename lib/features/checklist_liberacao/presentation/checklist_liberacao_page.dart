...
class _ChecklistLiberacaoPageState extends State<ChecklistLiberacaoPage> {
  ...
  Color _answerColor(BuildContext context, ChecklistAnswer value) {
  ...
  IconData _answerIcon(ChecklistAnswer value) {
    switch (value) {
      case ChecklistAnswer.sim:
  ...
  Widget _buildAnswerCell(
    ...
    String questionId,
    ChecklistAnswer value,
  ) {
    final selected = _answers[questionId] == value;
    final color = _answerColor(context, value);
  ...
  DataRow _buildQuestionRow(ChecklistQuestion question, BuildContext context) {
    return DataRow(
      cells: [
        DataCell(
          ConstrainedBox(
        ...
        DataCell(_buildAnswerCell(context, question.id, ChecklistAnswer.sim)),
        DataCell(_buildAnswerCell(context, question.id, ChecklistAnswer.nao)),
        DataCell(
          _buildAnswerCell(context, question.id, ChecklistAnswer.naoAplica),
        ),
      ],
    );
  }
                  ...
                  child: ConstrainedBox(
                    ...
                    child: DataTable(
                      ...
                      rows: group.questions
                          .map((question) => _buildQuestionRow(question, context))
                          .toList(),
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          final maxFormWidth = constraints.maxWidth.clamp(0.0, 1440.0);
          final spacing = 24.0;
          final columns = maxFormWidth >= 1200
              ? 3
              : maxFormWidth >= 820
                  ? 2
                  : 1;
          final fieldWidth = columns == 1
              ? maxFormWidth
              : (maxFormWidth - (columns - 1) * spacing) / columns;
